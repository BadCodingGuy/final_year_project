...
import '../../Services/teacher_database.dart';
...
class _SettingsFormState extends State<SettingsForm> {
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<TeacherUserData>(
      stream: TeacherDatabaseService(uid: user?.uid ?? '').userData,
      builder: (context, snapshot) {
        if (snapshot.hasData) {
          TeacherUserData? userData = snapshot.data;
          return Form(
            key: _formKey,
            child: Column(
              children: <Widget>[
                ...
                ElevatedButton(
                  ...
                  onPressed: () async {
                    if (_formKey.currentState?.validate() ?? false) {
                      await TeacherDatabaseService(uid: user?.uid ?? '').updateUserData(
